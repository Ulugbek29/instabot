...
async def link_handler(Mbot, message):
    ...
    try:
       ...
       if get_api['success'] == "false":
          return await message.reply("Неверный URL-адрес видео TikTok. Пожалуйста, попробуйте еще раз :)")
       if get_api['success'] == "ok":
          if get_api.get('result').get('hd'):
             try:
                 dump_file = await message.reply_video(get_api['result']['hd'],caption="Спасибо за использование - @OKAsave_bot")
             except KeyError:
                 pass 
             except Exception:
                 try:
                     ...
                     try:
                        ...
                        await message.reply_video(down_file,caption="Спасибо за использование - @OKAsave_bot")
                        await sndmsg.delete()
                        os.remove(down_file)
                     except:
                         return await message.reply("К сожалению, не удалось отправить файл вместо ссылки")
          else: 
             if get_api.get('result').get('sd'):
               try:
                   dump_file = await message.reply_video(get_api['result']['sd'],caption="Спасибо за использование - @OKAsave_bot")
               except KeyError:
                   pass
               except Exception:
                   try:
