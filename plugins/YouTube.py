...
async def _(Mbot,message):
    ...
    if "shorts" in link:
        ...
        return await message.reply("Посетите @FilmPrimiere\n")
    try:
  ...
  #          await message.reply(id[2])
        ...
        #    await message.reply_audio(fileLink)
            AForCopy = await message.reply_audio(fileLink,caption=f"[{id[3]}](https://youtu.be/{id[0]}) - {id[2]} Спасибо за использование - @OKAsave_bot",title=id[3].replace("_"," "),performer=id[2],thumb=thumnail,duration=id[4])
            if DUMP_GROUP:
        ...
        if LOG_GROUP:
               ...
               await message.reply(f"Посетите @FilmPrimiere\n")
               await Mbot.send_message(LOG_GROUP, traceback.format_exc())
